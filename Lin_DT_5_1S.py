# -*- coding: utf-8 -*-
"""
Created on Sun Jun 12 10:34:59 2016
...
def gettrain_timesplit(days,p,is_test=False):
    ...
    gap=np.load('train_gap_1.npy')
    poi=np.load('poi.npy')
    supply=np.load('train_total_1.npy')
    ...
    for T in time:
        for i in range(66):
            ...
            for k in days:             
                ...
                out.append(float(gap[T,i,k-1]+gap[T+1,i,k-1]+gap[T+2,i,k-1]+gap[T+3,i,k-1]+gap[T+4,i,k-1]+gap[T+5,i,k-1]+gap[T+6,i,k-1]+gap[T+7,i,k-1]+gap[T+8,i,k-1]+gap[T+9,i,k-1]))  #用于测试误差的的真实值
                t+=1
    return feature,out
...
def didi_train(model_type,p,compare=1):
    ...
    if model_type=='GBDT':
        ...
        if compare==1:
            ...
            print 'Lin_DT_2S:',R,R1,R2
            return result,r
        else:
            Result=refineresult2(result)
            np.save('./6_11_2S_2',Result)
