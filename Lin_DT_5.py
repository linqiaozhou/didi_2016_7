...
def gettrain_timesplit(days,p,is_test=False):
    ...
    poi=np.load('poi.npy')
    feature=np.zeros((len(days)*len(time)*66,25+3*mutlpy),dtype=float)
    #T=time[p-1]
    out=[]
    t=0
    for T in time:
...
def gettest_timesplit():
    ...
    for j in range(len(feature)):
        if feature[j,25]==-1 and feature[j,26]==-1 and feature[j,27]and feature[j,28]==-1 and feature[j,29]==-1 and feature[j,30]==-1:
