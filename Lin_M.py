# -*- coding: utf-8 -*-
"""
Created on Sat Jun 11 16:10:35 2016
...
def gettrain_timesplit(days,p,is_test=False):
    ...
    gap_10=np.load('train_gap.npy') 
    poi=np.load('poi.npy')
    supply=np.load('train_total_5.npy')
    feature=np.zeros((len(days)*len(time)*66,25+3*mutlpy+3*mutlpy+3),dtype=float)
    #T=time[p-1]
    out=[]
    t=0
    for T in time:
        for i in range(66):
            ...
            for k in days:             
                ...
                feature[t,25+3*mutlpy+3*mutlpy:]=gap_10[T/2-3:T/2,i,k-1]
...
def gettest_timesplit():
    ...
    supply=np.load('test_total_5.npy')
